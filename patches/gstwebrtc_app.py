...
class GSTWebRTCApp:
    ...
    def set_pointer_visible(self, visible):
        ...
        # Force cursor always visible — games in fullscreen hide it,
        # but we need it visible on the browser side for desktop use
        visible = True
    ...
    def start_pipeline(self, audio_only=False):
        ...
        # Accept both SUCCESS and ASYNC (ASYNC means it's transitioning, which is fine).
        res = self.pipeline.set_state(Gst.State.PLAYING)
        if res not in (Gst.StateChangeReturn.SUCCESS, Gst.StateChangeReturn.ASYNC, Gst.StateChangeReturn.NO_PREROLL):
